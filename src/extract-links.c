        ...
        for (; m != NULL; m = m->next)
        {
            ...
            for (lcost = 0; lcost <= cost; lcost++)
            {
                ...
                if (Lmatch)
                {
                    ...
                    if (le->multi)
                        ls[1] = parse_set(ld, d, lw, w, le, d->left->next, lcost, pi);
                    if (d->left->multi)
                        ls[2] = parse_set(ld, d, lw, w, le->next, d->left, lcost, pi);
                    if (le->multi && d->left->multi)
                        ls[3] = parse_set(ld, d, lw, w, le, d->left, lcost, pi);
                }
                if (Rmatch)
                {
                    rs[0] = parse_set(d, rd, w, rw, d->right->next, re->next, rcost, pi);
                    if (d->right->multi)
                        rs[1] = parse_set(d, rd, w, rw, d->right, re->next, rcost, pi);
                    if (re->multi)
                        rs[2] = parse_set(d, rd, w, rw, d->right->next, re, rcost, pi);
                    if (d->right->multi && re->multi)
                        rs[3] = parse_set(d, rd, w, rw, d->right, re, rcost, pi);
                }
...
int advance_linkage(Parse_info *pi, Parse_set *set)
{
    /* Advance the "current" linkage to the next one
       ...
       starting state. */
    if (set == NULL)
        return 1; /* probably can't happen */
    if (set->first == NULL)
