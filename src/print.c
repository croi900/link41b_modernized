...
char *linkage_print_links_and_domains(Linkage linkage)
{
    ...
    {
        if (linkage_get_link_lword(linkage, link) == -1)
            continue;
...
char *build_linkage_postscript_string(Linkage linkage)
{
    ...
    if (dict->left_wall_defined)
    {
        ...
        print_word_0 = (((!suppressor_used) && (N_wall_connectors != 0)) || (N_wall_connectors > 1) || opts->display_walls);
    }
    else
    {
        print_word_0 = TRUE;
    ...
    if (dict->right_wall_defined)
    {
        ...
        print_word_N = (((!suppressor_used) && (N_wall_connectors != 0)) || (N_wall_connectors > 1) || opts->display_walls);
    }
    else
    {
        print_word_N = TRUE;
    ...
    for (link = 0; link < N_links; link++)
    {
        if (!print_word_0 && (ppla[link]->l == 0))
            continue;
        if (!print_word_N && (ppla[link]->r == linkage->num_words - 1))
            continue;
        if (ppla[link]->l == -1)
            continue;
        if ((j % 7 == 0) && (j > 0))
    ...
    {
        if (j > 0)
            append_string(string, " %d", row_starts[j]);
        else
            append_string(string, "%d", row_starts[j]);
    }
    append_string(string, "]\n");
...
char *linkage_print_diagram(Linkage linkage)
{
    ...
    for (link_length = 1; link_length < N_words_to_print; link_length++)
    {
        for (j = 0; j < N_links; j++)
        {
            ...
            if (!print_word_N && (ppla[j]->r == linkage->num_words - 1))
                continue;
            ...
            if (opts->display_link_subscripts)
                ...
                for (t = connector; isupper((int)*t); t++)
