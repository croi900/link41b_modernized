...
{
    int w;
    if (!sentence_contains(sent, "either"))
        return;
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "either") != 0)
            continue;
    ...
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "or") != 0)
            continue;
        sent->word[w].d = glom_aux_connector(sent->word[w].d, EITHER_LABEL, FALSE);
    }
}
...
static void construct_neither(Sentence sent)
{
    ...
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "neither") != 0)
            continue;
        sent->word[w].d = catenate_disjuncts(
    ...
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "nor") != 0)
            continue;
        sent->word[w].d = glom_aux_connector(sent->word[w].d, NEITHER_LABEL, TRUE);
    }
}
...
static void construct_notonlybut(Sentence sent)
{
    ...
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "not") != 0)
            continue;
        sent->word[w].d = catenate_disjuncts(
    ...
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "but") != 0)
            continue;
        sent->word[w].d = glom_aux_connector(sent->word[w].d, NOT_LABEL, FALSE);
...
{
    int w;
    if (!sentence_contains(sent, "both"))
        return;
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "both") != 0)
            continue;
        sent->word[w].d = catenate_disjuncts(
    ...
    for (w = 0; w < sent->length; w++)
    {
        if (strcmp(sent->word[w].string, "and") != 0)
            continue;
