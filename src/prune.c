...
int prune_match(Connector *a, Connector *b, int aw, int bw)
{
    ...
    */
    ...
    if (aw == 0 && bw == 0)
    { /* probably not necessary, as long as effective_dist[0][0]=0 and is defined */
        dist = 0;
    }
    else
    {
...
void free_S(void)
{
    ...
    {
        if (table[i] == NULL)
            continue; /* a prehaps stupid optimization */
...
int disjunct_matches_alam(Disjunct *d1, Disjunct *d2)
{
    ...
    {
        if (!connector_matches_alam(e1, e2))
            break;
        e1 = e1->next;
        e2 = e2->next;
    }
...
Disjunct *eliminate_duplicate_disjuncts(Disjunct *d)
{
    ...
    */
    ...
    if ((verbosity > 2) && (count != 0))
        printf("killed %d duplicates\n", count);
    return d;
}
...
int disjuncts_equal(Disjunct *d1, Disjunct *d2)
{
    ...
    while ((e1 != NULL) && (e2 != NULL))
    {
        if (!connectors_equal_prune(e1, e2))
            break;
        e1 = e1->next;
        e2 = e2->next;
    }
...
int power_prune(Sentence sent, int mode, Parse_Options opts)
{
    ...
    while (!parse_options_resources_exhausted(opts))
    {
        ...
        {
            if (parse_options_resources_exhausted(opts))
                break;
            for (d = sent->word[w].d; d != NULL; d = d->next)
            {
                if (d->left == NULL)
...
int match_in_cms_table(char *pp_match_name)
{
    ...
    {
        if (post_process_match(pp_match_name, cms->name))
            return TRUE;
    }
    return FALSE;
}
                ...
                for (dir = 0; dir < 2; dir++)
                {
                    for (c = ((dir) ? (d->left) : (d->right)); c != NULL; c = c->next)
                    {
                        for (i = 0; i < knowledge->n_contains_one_rules; i++)
                        {
                            ...
                            if (!post_process_match(selector, c->string))
                                continue;
...
void pp_and_power_prune(Sentence sent, int mode, Parse_Options opts)
{
    ...
    for (;;)
    {
        ...
        if (parse_options_resources_exhausted(opts))
            break;
        if (power_prune(sent, mode, opts) == 0)
            break;
    }
}
