...
int find_one_non_idiom_node(Dict_node *p, Dict_node *dn, char *s)
{
	...
	*/
	...
	if (m <= 0)
	{
		if (find_one_non_idiom_node(dn, dn->left, s))
			return TRUE;
	}
	...
	if (m >= 0)
	{
		if (find_one_non_idiom_node(dn, dn->right, s))
			return TRUE;
	}
	return FALSE;
}
...
int delete_dictionary_words(Dictionary dict, char *s)
{
	...
	*/
	...
	if (!find_one_non_idiom_node(NULL, dict->root, s))
		return FALSE;
	for (;;)
	{
